...
export default function CodeAssessmentWorkspace({ mode = "placement", practiceProblemId, initialLanguage = "JavaScript", onLanguageChange }) {
  ...
  const [language, setLanguage] = useState(() => Object.hasOwn(starters, initialLanguage) ? initialLanguage : "JavaScript");
  ...
  const execute = async (submit) => {
    ...
    lock.current = true; setBusy(submit ? "Submitting hidden tests..." : "Compiling and running sample tests..."); setError(""); setResult(null);
    try {
  ...
  return <section className="surface-card space-y-5 rounded-3xl p-6">
    ...
    <p role="status">{busy}</p>{error && <p role="alert" className="text-rose-500">{error} Your draft is saved. You can retry or request a code review; review feedback does not count as executed tests.</p>}
    {result?.submitted && <div aria-live="polite" className={`flex items-center gap-2 rounded-xl border p-4 text-lg font-extrabold ${result.status === "passed" ? "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-300" : "border-rose-200 bg-rose-50 text-rose-700 dark:border-rose-900 dark:bg-rose-950/30 dark:text-rose-300"}`}>{result.status === "passed" ? <CheckCircle2 size={22}/> : <XCircle size={22}/>}{result.status === "passed" ? "Passed - the submission met the 80% threshold." : "Not passed - review failed tests below."}</div>}
