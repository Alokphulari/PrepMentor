...
export default function CodeAssessmentWorkspace({ mode = "placement" }) {
  const { user } = useAuth();
  ...
  const problem = problems.find((item) => item.id === selected) || (mode === "placement" ? problems.find((item) => item.difficulty === availableLevel) : problems[0]);
  ...
  return <section className="surface-card space-y-5 rounded-3xl p-6">
    ...
    {mode === "practice" && <label>Problem<select className="ml-3 rounded border p-2 dark:bg-gray-900" value={problem?.id || ""} onChange={(event) => { setSelected(event.target.value); setResult(null); }}>{problems.map((item) => <option key={item.id} value={item.id}>{item.difficulty}: {item.title}</option>)}</select></label>}
    {problem && !(mode === "placement" && result?.submitted) && <>
      ...
      <label className="block">Language<select disabled={Boolean(busy)} value={language} onChange={(event) => setLanguage(event.target.value)} className="ml-3 rounded border p-2 dark:bg-gray-900">{Object.keys(starters).map((item) => <option key={item}>{item}</option>)}</select></label>
      ...
      <div className="flex gap-3"><button disabled={Boolean(busy)} onClick={() => execute(false)} className="rounded-xl border px-5 py-3 font-bold disabled:opacity-50">Run sample tests</button><button disabled={Boolean(busy)} onClick={() => execute(true)} className="rounded-xl bg-indigo-600 px-5 py-3 font-bold text-white disabled:opacity-50">Submit hidden tests</button></div>
    </>}
    {!problem && !result && <p>Complete the previous Placement stage, or finish your Learning Hub remediation to unlock a retake. <Link className="text-indigo-500" to="/placement">View progress</Link></p>}
    <p role="status">{busy}</p>{error && <p role="alert" className="text-rose-500">{error}</p>}
    {result && <div aria-live="polite" className="space-y-3 rounded-xl border p-5"><h3 className="font-bold">{result.submitted ? "Hidden test results" : "Sample results"}: {result.passedTests}/{result.totalTests} passed</h3><p>Runtime: {result.runtime.toFixed(3)} seconds · Memory: {result.memory} KB</p>{result.failureReason && <p>{result.failureReason}</p>}{result.submitted && <p>Score: {result.score}%</p>}
