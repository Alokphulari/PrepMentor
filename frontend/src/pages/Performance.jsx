...
function Performance() {
  ...
  const analytics = useMemo(() => {
    ...
    return { average, best, recent, categories, improvement };
  }, [attempts]);

  if (!attempts.length) return <div className="space-y-7"><header><p className="text-sm font-bold text-indigo-600 dark:text-indigo-400">Performance analytics</p><h1 className="mt-1 text-3xl font-extrabold tracking-tight sm:text-4xl">Turn practice into progress.</h1></header><PerformanceAnalysis history={history}/><section className="surface-card rounded-3xl px-6 py-16 text-center"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-500 dark:bg-indigo-950/40"><Sparkles size={29}/></div><h2 className="mt-5 text-xl font-extrabold">Your analytics will appear here</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-gray-500">Complete your first practice session to unlock score trends, category averages, and improvement insights.</p><Link to="/practice/aptitude" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-bold text-white hover:bg-indigo-700">Start a session <ArrowRight size={16}/></Link></section></div>;
