import LearningResources, { LearningResourceLibrary } from "../components/LearningResources";
...
import { ArrowLeft, ArrowRight, BookOpenCheck, Check, CheckCircle2, Lightbulb, LockKeyhole, PlayCircle, Target, TrendingUp } from "lucide-react";
import { useLocation, useNavigate } from "react-router-dom";
import { usePlacement } from "../context/PlacementContext";
import { useAuth } from "../context/AuthContext";
import { getAccountStorageKey, readStorage, writeStorage } from "../utils/storage";
import { getWeakLearningTopics, normalizeCompletedTopics, normalizeVisitedReferences } from "../utils/learningPlan";
...
function LearningWorkspaceContent() {
  ...
  const { user } = useAuth();
  const { placementState, retryAptitude, retryCoding, applyServerPlacement } = usePlacement();
  const storageKey = getAccountStorageKey(SESSION_KEY, user);
  const [session] = useState(() => {
    const incoming = location.state;
    if (incoming?.topicPerformance) {
      const saved = readStorage(storageKey, {});
      const same = ["topicPerformance", "module", "placementLevel", "resultId"].every((key) => JSON.stringify(saved[key]) === JSON.stringify(incoming[key]));
      return same ? { ...incoming, ...saved } : incoming;
    }
    return readStorage(storageKey, {});
  });
  const weakTopics = useMemo(() => {
    return getWeakLearningTopics(session.topicPerformance, defaultTopics).map((item) => ({ ...item, lesson: session.learningPlan?.find((plan)=>plan.topic===item.topic)?.tasks || buildLesson(item.topic) }));
  }, [session.topicPerformance, session.learningPlan]);
  const [selectedTopic, setSelectedTopic] = useState(() => session.selectedTopic || weakTopics[0]?.topic || "");
  ...
  const validPlacementModule = ["aptitude", "coding", "interview"].includes(session.module);
  const validPlacementLevel = ["easy", "medium", "hard", "status"].includes(session.placementLevel);
  ...
  const handleContinue = async () => {
    ...
    if(placementMode && ["coding","interview"].includes(session.module) && hasRemoteApi){
      try{const response=await authorizedRequest(session.module === "interview" ? "/api/interview-remediation" : "/api/code/remediation",{method:"POST",body:JSON.stringify({level:session.placementLevel,reflection:Object.values(reflections).join(" ")})});applyServerPlacement(response.placementState);navigate(session.module === "interview"?"/interview/setup":"/placement/coding",{state:{mode:"placement"}});}catch(error){setRetryError(error.message);}return;
    }
    if (placementMode) {
  ...
  return <div className="space-y-7">{retryError&&<p role="alert">{retryError}</p>}<header className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="text-sm font-bold text-indigo-600 dark:text-indigo-400">Learning Hub</p><h1 className="mt-1 text-3xl font-extrabold tracking-tight sm:text-4xl">Personalized Learning Hub</h1><p className="mt-2 max-w-2xl text-gray-500 dark:text-gray-400">Review the ideas behind missed questions, complete each focus topic, and return when you are ready.</p></div>{placementMode&&<div className="rounded-2xl border border-indigo-200 bg-indigo-50 px-4 py-3 text-sm dark:border-indigo-900 dark:bg-indigo-950/40"><p className="font-bold text-indigo-800 dark:text-indigo-200">Placement remediation</p><p className="mt-1 capitalize text-indigo-600 dark:text-indigo-300">{session.module} · {session.placementLevel} level</p></div>}</header>{!session.topicPerformance&&<><PersonalizedLearningPlan/><LearningResourceLibrary/></>}
    <section className="surface-card rounded-3xl p-6"><div className="flex items-center justify-between gap-5"><div><p className="text-sm font-bold">Learning progress</p><p className="mt-1 text-xs text-gray-500">{completed.size} of {weakTopics.length} focus topics complete</p></div><span className="text-2xl font-black text-indigo-600 dark:text-indigo-300">{progress}%</span></div><div className="mt-4 h-2.5 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800"><div className="h-full rounded-full bg-indigo-500 transition-all duration-500" style={{width:`${progress}%`}}/></div></section>
    <div className="grid items-start gap-6 lg:grid-cols-[0.72fr_1.28fr]"><aside className="surface-card rounded-3xl p-4"><p className="px-2 pb-3 text-xs font-black uppercase tracking-[0.18em] text-gray-400">Focus topics</p><div className="space-y-2">{weakTopics.map((item)=><button key={item.topic} type="button" onClick={()=>setSelectedTopic(item.topic)} className={`flex w-full items-center gap-3 rounded-2xl p-3 text-left transition ${selectedTopic===item.topic ? "bg-indigo-50 dark:bg-indigo-950/45" : "hover:bg-gray-50 dark:hover:bg-gray-800/50"}`}><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${completed.has(item.topic) ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-950" : "bg-rose-50 text-rose-500 dark:bg-rose-950/40"}`}>{completed.has(item.topic)?<Check size={19}/>:<Target size={19}/>}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{item.topic}</p><p className="mt-0.5 text-xs text-gray-400">Previous score: {item.percentage}% | Priority: {item.percentage < 60 ? "High" : "Medium"}</p></div><ArrowRight size={15} className="text-gray-300"/></button>)}</div>{weakTopics.length===0&&<div className="rounded-2xl bg-emerald-50 p-5 text-center dark:bg-emerald-950/30"><CheckCircle2 className="mx-auto text-emerald-600"/><p className="mt-2 text-sm font-bold text-emerald-700 dark:text-emerald-300">No weak topics detected</p></div>}</aside>
      {selected ? <main className="surface-card rounded-3xl p-6 sm:p-8"><div className="flex items-start justify-between gap-5"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-600 dark:bg-indigo-950 dark:text-indigo-300"><BookOpenCheck size={24}/></div><span className={`rounded-full px-3 py-1.5 text-xs font-bold ${completed.has(selected.topic)?"bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300":"bg-amber-50 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300"}`}>{completed.has(selected.topic)?"Completed":"In progress"}</span></div><p className="mt-6 text-xs font-black uppercase tracking-[0.18em] text-indigo-600 dark:text-indigo-400">Focused lesson</p><h2 className="mt-2 text-2xl font-extrabold">{selected.topic}</h2><p className="mt-3 leading-7 text-gray-500 dark:text-gray-400">Recommended because your latest score in this topic was {selected.percentage}%. Use this short framework to approach the topic deliberately. Say each step aloud as if explaining it to an interviewer.</p><div className="mt-7 space-y-3">{selected.lesson.map((point,index)=><div key={point} className="flex gap-4 rounded-2xl border border-gray-200 p-4 dark:border-gray-700"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gray-100 text-sm font-black text-gray-600 dark:bg-gray-800 dark:text-gray-300">{index+1}</span><p className="pt-1 text-sm font-medium leading-6">{point}</p></div>)}</div><div className="mt-7"><p className="text-xs font-black uppercase tracking-[0.16em] text-gray-400">Required references</p><p className="mt-2 text-sm text-gray-500 dark:text-gray-400">Open at least one reference for this topic before marking it complete.</p><LearningResources topic={selected.topic} onOpen={()=>openReference(selected.topic)}/>{visitedReferences.has(selected.topic)&&<p className="mt-3 flex items-center gap-2 text-xs font-bold text-emerald-600"><Check size={15}/> Reference opened. Complete your reflection to finish this topic.</p>}</div><div className="mt-7 rounded-2xl bg-amber-50 p-5 dark:bg-amber-950/30"><p className="flex items-center gap-2 font-bold text-amber-800 dark:text-amber-200"><Lightbulb size={18}/> Quick reflection</p><p className="mt-2 text-sm leading-6 text-amber-700 dark:text-amber-300">Before marking this complete, create one example of your own and explain why your approach works.</p></div><button type="button" disabled={completed.has(selected.topic)||!visitedReferences.has(selected.topic)||countLearningWords(reflections[selected.topic])<20} onClick={completeTopic} className="mt-7 flex items-center gap-2 rounded-xl bg-indigo-600 px-5 py-3 text-sm font-bold text-white transition hover:-translate-y-0.5 hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-gray-300 dark:disabled:bg-gray-700"><CheckCircle2 size={17}/>{completed.has(selected.topic)?"Topic completed":visitedReferences.has(selected.topic)?"Mark topic complete":"Open a reference first"}</button></main> : <main className="surface-card rounded-3xl p-10 text-center"><TrendingUp className="mx-auto text-emerald-500" size={34}/><h2 className="mt-4 text-xl font-bold">You are ready to continue</h2><p className="mt-2 text-gray-500">No additional remediation is required for this session.</p></main>}</div>
...
export default function LearningWorkspace() {
  const location = useLocation();
  return <LearningWorkspaceContent key={location.key}/>;
}
