...
function InterviewResult() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6"><section className="surface-card relative overflow-hidden rounded-3xl p-8 sm:p-10"><div className="absolute -right-16 -top-16 h-56 w-56 rounded-full bg-indigo-500/10 blur-3xl"/><div className="relative flex flex-col gap-7 sm:flex-row sm:items-center"><div className="flex h-28 w-28 shrink-0 items-center justify-center rounded-full border-[10px] border-indigo-100 text-3xl font-black text-indigo-600 dark:border-indigo-950 dark:text-indigo-300">{score}%</div><div><p className="flex items-center gap-2 text-sm font-bold text-emerald-600"><CheckCircle2 size={17}/> Interview complete</p><h1 className="mt-2 text-3xl font-extrabold tracking-tight">{getInterviewHeadline(score)}</h1><p className="mt-2 text-gray-500 dark:text-gray-400">{result.role || "Software Engineer"} · {result.type || "Mixed"} interview</p></div></div></section>
    <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4 text-sm text-indigo-700 dark:border-indigo-900 dark:bg-indigo-950/30 dark:text-indigo-300"><strong>Evaluation mode:</strong> {result.evaluationMode || "Local practice rubric"}. {result.evaluationMode?.startsWith("AI semantic") ? "AI evaluated answer relevance, reasoning and technical content. Fluency is a communication proxy, not psychological confidence." : "This fallback measures response completeness only, not technical correctness."}</div><section className="grid gap-4 sm:grid-cols-2">{feedback.metrics.map(({key,label,value})=><div key={key} className="surface-card interactive-card rounded-2xl p-5"><div className="flex items-center justify-between"><p className="font-bold">{key==="confidence"?"Communication fluency (proxy)":label}</p><span className="text-lg font-black text-indigo-600 dark:text-indigo-300">{value}%</span></div><div className="mt-4 h-2 rounded-full bg-gray-100 dark:bg-gray-800"><div className="h-full rounded-full bg-indigo-600" style={{width:`${value}%`}}/></div></div>)}</section>
    <section className="grid gap-5 lg:grid-cols-2"><div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-900 dark:bg-emerald-950/30"><h2 className="flex items-center gap-2 font-bold text-emerald-800 dark:text-emerald-200"><Award size={20}/> Strengths</h2><ul className="mt-4 list-disc space-y-3 pl-5 text-sm leading-6 text-emerald-700 dark:text-emerald-300">{(result.strengths?.length?result.strengths:feedback.strengths).map((strength)=><li key={strength}>{strength}</li>)}</ul></div><div className="rounded-3xl border border-amber-200 bg-amber-50 p-6 dark:border-amber-900 dark:bg-amber-950/30"><h2 className="flex items-center gap-2 font-bold text-amber-800 dark:text-amber-200"><TrendingUp size={20}/> Next focus</h2><ul className="mt-4 list-disc space-y-3 pl-5 text-sm leading-6 text-amber-700 dark:text-amber-300">{(result.recommendations?.length?result.recommendations:feedback.recommendations).map((recommendation)=><li key={recommendation}>{recommendation}</li>)}</ul></div></section>
    ...
    {result.questionFeedback?.length>0&&<section className="surface-card space-y-5 rounded-3xl p-6"><h2 className="font-bold">Question-by-question feedback</h2>{result.questionFeedback.map((item,i)=><article key={i}><h3 className="font-semibold">{i+1}. {item.question} ({item.score}%)</h3><p className="mt-3 text-xs font-bold uppercase text-gray-500">Your answer</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6">{item.answer || "Answer unavailable for this older report."}</p><p className="mt-3 text-sm leading-6">{item.feedback}</p><h4 className="mt-4 font-bold text-indigo-600 dark:text-indigo-300">Strong sample answer</h4><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-gray-500 dark:text-gray-400">{item.idealAnswer || item.betterApproach || "A sample answer is unavailable for this offline evaluation."}</p></article>)}</section>}
